"""
Batch-generate Stable Diffusion 3.5 Large (fp8 scaled, real CFG) images
for OmniPromptStyle-CheatSheet.

Same target list, same 4-panel-grid approach as generate_krea2raw_batch.py.
SD3.5 Large is the full, non-distilled 8B model (not the 4-step "Turbo"
variant) -- steps/cfg/sampler match the official ComfyUI example workflow
(sd3.5_simple_example.json / comfyanonymous.github.io/ComfyUI_examples/sd3).

Single all-in-one checkpoint (CLIP-L/G/T5-XXL + VAE baked in) -- loaded
via CheckpointLoaderSimple like any SD checkpoint, no separate
CLIPLoader/VAELoader/UNETLoader split needed (unlike Krea2/Flux2).

Reuses the same category-aware negative prompt as the Krea2 RAW script:
Photography-tagged artists don't get the anti-photorealism terms, since
a photographic look IS their actual style.

Model setup:
  - sd3.5_large_fp8_scaled.safetensors (checkpoints/sd35, ~14.9GB)

Output: ./output/img/style/SD35/Large_FP8/<Image>.webp

Usage:
    python generate_sd35_batch.py             # run the whole batch
    python generate_sd35_batch.py --limit 2    # test on the first N only
    python generate_sd35_batch.py              # re-run any time to resume /
...
OUTPUT_ROOT = SCRIPT_DIR / "output" / "img" / "style" / "SD35" / "Large_FP8"
COMFY_OUTPUT_DIR = Path(r"G:\StabilityMatrix-win-x64\Data\Packages\ComfyUI\output")
LOG_FILE = SCRIPT_DIR / "generation_log_sd35.jsonl"
...
CHECKPOINT_NAME = "sd35\\sd3.5_large_fp8_scaled.safetensors"
STEPS = 20
CFG = 4.0
SAMPLER_NAME = "euler"
...
def negative_prompt_for(category: str) -> str:
    ...
    failures seen in the Krea2 RAW testing."""
...
def build_graph(prompt_text: str, seed: int, filename_prefix: str, negative_prompt: str) -> dict:
    return {
        ...
        "6": {
            ...
            "inputs": {"clip": ["193", 1], "text": prompt_text},
        },
        "7": {
            "class_type": "CLIPTextEncode",
            "inputs": {"clip": ["193", 1], "text": negative_prompt},
        },
        "162": {
        ...
        "164": {
            ...
            "inputs": {"samples": ["163", 0], "vae": ["193", 2]},
        },
        "999": {
...
def main():
    ...
    for i, artist in enumerate(artists, 1):
        ...
        safe_base = "sd35_" + re.sub(r"[^A-Za-z0-9_-]", "_", image_name.rsplit(".", 1)[0])
